...
function arbiter_subscribe_to_match($id_match, $id_arbitres, $c){
    arbiter_unsubscribe_to_match($id_match, $id_arbitres, $c);
    $sql = ("INSERT INTO matchs_arbitres (id_matchs, id_arbitres) VALUES ('$id_match', '$id_arbitres')");
    if(mysqli_query($c,$sql)){
        return true;
    }
    else{
...
function get_arbiter_by_match_id($id_match, $c){
    $sql = ("SELECT id_arbitres FROM matchs_arbitres WHERE id_matchs = '$id_match' ");
    $result = mysqli_query($c,$sql);
    $otm_list= array ();
    $loop = 0;
    while ($donnees = mysqli_fetch_assoc($result))
    {
        $otm_list[$loop] = $donnees['id_arbitres'];
        $loop++;
    }
    return $otm_list;
}
...
function Delete_all_arbiter_from_match($id_match, $c){
    $sql = ("DELETE FROM matchs_arbitres WHERE id_matchs = '$id_match'");
    if(mysqli_query($c,$sql)){
        return true;
    }
    else{
